 ...
 * @date 2018/10/22
 */
class ParentStudyPlanView(context: Context?, attrs: AttributeSet?, defStyleAttr: Int) : RelativeLayout(context, attrs, defStyleAttr) {
    ...
    fun refreshView() {
            ...
            override fun onScrolled(recyclerView: RecyclerView?, dx: Int, dy: Int) {
                ...
                if (view != null) {
                    ...
                    if (view.left <= mVieDistance) {
                        title.x = (-(mVieDistance - view.left) - viewNow.x).toFloat()
                        title2.x = 0f
                    } else {
                        title.x = -viewNow.x
                    }
        ...
        recy_study_plan.adapter = ParentStudyPlanAxisAdapter(
